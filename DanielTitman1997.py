...
-----------------------------------------------------------------
    ...
    3. Iterate through a list of June dates (list of date_jun in ascending order) and 
       ...
       a twist:
           ...
           v. Apply tge FFPortfolios() method. 
...
sizebtm_p['HML_DT97'] = (1/2)*(sizebtm_p['SH'] + sizebtm_p['BH']) - \
          ...
          'aate_m' = monthly or 'date' for daily returns of the factors 
...
-------
    1. CRSPmonthlyFF5.csv : 
        ...
        Only PERMNOs with share code (SHRCD) 10 or 11 are included.
    2. FF3_monthly.csv : 
...
# Subset only NYSE, AMEX, NASDAQ stocks
firmchars = firmchars[firmchars['EXCHCD'].isin([1, 2, 3])]
# Isolate dates after (including) 196306
...
# Drop the last formation date which corresponds to 202106
...
for formation_date in fdates['date_jun']:
    ...
    if do_MKT:
        ...
        crspmkt = pd.merge(crspm5Y, firmchars5Y[['date_jun', 'PERMNO', 'ME']], on = ['date_jun', 'PERMNO']).dropna()
        
        # Calculate the market return 
        mkt_DT97 = crspmkt.groupby(by = 'date_m').agg( {'RET' : lambda x: WeightedMean(x, df=crspmkt, weights = 'ME') } )
        # Rename the column
